...
function SearchBoxItem() {
    ...
    let renderItem = (
        <div className="card search-box-item shadow">
            <div className="card-header text-dark border-bottom">{searchStudent.length} Candidate Found</div>
            <div className="card-list-item">
            {searchStudent.map((item, index) => 
                <ul className="list-group list-group-flush" key={index}>
                    <li className="list-group-item text-dark" style={{cursor: "pointer"}} onClick={() => handleChange(item)}>
